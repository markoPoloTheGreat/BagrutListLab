...
public class Program
{
    ...
    public static int AvgGradeInLst(Node<Student> first)
    {//עוברת על התלמידים ומחשבת את הממוצע שלהם
        Node<Student> currNode = first;
        int sum = 0;
        int count = 0;
        while (currNode != null)
        {
            sum += currNode.GetValue().GetGrade();
            count++;
            currNode = currNode.GetNext();
        }
        return sum / count;
    ...
    {
        Group minG = GroupArr[0];
        for (int i = 1; i < GroupArr.Length; i++)//find the smallest group
        {
            if (GroupArr[i].GetKamut() < minG.GetKamut())
            {
                minG = GroupArr[i];
            }
        }
        bool isInAll = true;
        for (int i = 0; i < minG.GetKamut(); i++)
        {
            for (int j = 0; j < GroupArr.Length; j++)
            {
                if (!GroupArr[j].IsIncluded(minG.GetNamesArr()[i]))
                {
                    isInAll = false;
                }
